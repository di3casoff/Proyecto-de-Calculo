...
class PreguntasApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Aplicación Interactiva de Cálculo")
        ...
        self.boton_preparar_img = ImageTk.PhotoImage(Image.open("BotonPrepararTema.png"))
        ...
        self.boton_preparar = tk.Button(
            self.interfaz_juego,
            image=self.boton_preparar_img,
            command=lambda: [
                self.reproducir_sonido_boton(),
                self.seleccionar_tema(),
            ],
            borderwidth=0,
            bg="#9e3d3d",
            activebackground="#9e3d3d",
        )
        self.boton_preparar.pack(pady=10)
    ...
    def seleccionar_tema(self):
        ...
        threading.Thread(target=self._seleccionar_tema_animacion).start()

    def _seleccionar_tema_animacion(self):
        for _ in range(5):
            time.sleep(0.3)
    ...
    def mostrar_ejercicio(self):
        if self.tema_actual is None:
            messagebox.showerror(
                "Error",
                "⚠️ Debes preparar un tema antes de mostrar un ejercicio.",
            )
            return
